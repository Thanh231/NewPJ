...
public static partial class StaticUtils
{
	...
	public static string GetFrameworkPath()
	{
		...
		{
			return GetFrameworkPathInClientProject();
		}
	}

	private static string GetFrameworkPathInClientProject()
	{
		var parentFolder = $"{GetProjectPath()}/Library/PackageCache";
		const string packageName = "com.hoainam.gameframework";
		var allFolderPath = Directory.GetDirectories(parentFolder);
		foreach (var folderPath in allFolderPath)
		{
		...
		throw new Exception($"no folder start with {packageName} within folder {parentFolder}");
	}
	...
	//while the file is written, if there is any exception/crash, 
	//the file will be left empty, so it is better to write to a temp file first,
	//then rename it to the target file
	private static void OpenFileForWrite(string path, UnityAction<Stream> callback, bool isAbsolutePath)
	{
		path = GetAbsolutePath(path, isAbsolutePath);
	...
	{
		OpenFileForWrite(path, (Stream stream) =>
		{
	...
	public static List<string> GetAllFileNameInFolder(string path, bool isAbsolutePath = false)
	{
		path = GetAbsolutePath(path, isAbsolutePath);
		var result = new List<string>();
		foreach (var entry in Directory.GetFiles(path, "*", SearchOption.TopDirectoryOnly))
		{
	...
	public static List<string> GetAllFolderNameInFolder(string path, bool isAbsolutePath = false)
	{
		path = GetAbsolutePath(path, isAbsolutePath);
		var result = new List<string>();
