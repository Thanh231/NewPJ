...
public class PigComponent : MonoBehaviour
{
    ...
    public bool isMovingInLane = false;
    ...
    public void Initialize(string color, int bulletCount, int laneIndex, Color lineColor, float _speed, List<Transform> paths, bool isHidden)
    {
        ...
        bulletText.fontMaterial.renderQueue = 3500;
    ...
    {
        return (currentState == PigState.InLane || currentState == PigState.InQueue) && isOnTop && !isMovingInLane;
    }
    ...
    {
        StartNewLogic(async token => await MoveCoroutine(newLocalPos, token));
    }
    private async UniTask MoveCoroutine(Vector3 targetLocalPos, System.Threading.CancellationToken token)
    {
    ...
    private async UniTask JumpArcCoroutine(Vector3 startPos, Vector3 endPos, float duration, System.Threading.CancellationToken token, MMF_Player jumpFeedback = null)
    {
        ...
        while (timeSpent < duration)
        {
            ...
            float percent = Mathf.Clamp01(timeSpent / duration);

            Vector3 linearPos = Vector3.Lerp(startPos, endPos, percent);

            float arcHeight = 2f;
            float arcOffset = Mathf.Sin(percent * Mathf.PI) * arcHeight;

            Vector3 finalPos = linearPos;
    ...
    public void MoveToStack(Vector3 targetPos, Action onComplete = null)
    {
        StartNewLogic(async token => await MoveToStackCoroutine(targetPos, onComplete, token));
    }

    private async UniTask MoveToStackCoroutine(Vector3 targetPos, Action onComplete, System.Threading.CancellationToken token)
    {
        Vector3 start = rb.position;
        ...
        ChangeState(PigState.CanMove);
        onComplete?.Invoke(); // ← callback sau khi trượt xong
    }
    private async UniTask MoveInQueueCoroutine(Vector3 targetPos, Quaternion targetRot, System.Threading.CancellationToken token)
    {
        ...
        float elapsed = 0;
        // ChangeState(PigState.InQueue);
        bool isCanLandAnim = Vector3.Distance(new Vector3(startPos.x, 0, startPos.z), new Vector3(targetPos.x, 0, targetPos.z)) > 0.1f;
        while (elapsed < duration)
        {
            if (token.IsCancellationRequested) return;
    ...
    {
        invalidFB.PlayFeedbacks();
    }

    public void ApplyTemporarySlowdown(float duration = 0.2f, float slowMultiplier = 0.3f)
    {
        if (currentState != PigState.OnConveyor && currentState != PigState.Shooting) return;
        if (_isSlowedDown)
        {
            _slowdownTimer = duration; // refresh nếu bị đụng tiếp trong lúc đang chậm
            return;
        }
        _isSlowedDown = true;
        _slowdownTimer = duration;
        SetConveyorSpeedMultiplier(isRush ? 2f * slowMultiplier : slowMultiplier);
    }
}
