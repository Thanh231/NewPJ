...
public class HandlePigBehavior : SingletonMonoBehaviour<HandlePigBehavior>
{
    ...
    private float jumpFromLaneSpeed = 0.3f;
    private float jumpFromQueueSpeed = 0.3f;
    ...
    private List<Transform> availablePlatesList = new List<Transform>();
    ...
    private bool inTutorialFullSlot = false; // this flag is used to prevent multiple triggering of the full slot tutorial when clicking pigs rapidly
    ...
    public void UseItemAddTray()
    {
        ...
        availablePlatesList.Add(plate);
        RearrangePlatesInTray();
    }
    private void InitializePlates()
    {
        for (int i = 0; i < availablePlatesList.Count; i++)
        {
            if (availablePlatesList[i] != null)
            {
                Destroy(availablePlatesList[i].gameObject);
            }
        }
        availablePlatesList.Clear();
    ...
    {
        if (pigsInTempQueue.Contains(pig) || pig.currentState == PigState.Destroying) return;
        ...
        Vector3 targetPos = startTempQueuePos.position + (Vector3.right * (index * 0.85f));
    ...
    public void SelectPig(PigComponent pig)
    {
        ...
        if (pig.IsLinkedPig())
        {
            ...
            List<PigComponent> pigsToProcess = linkedPigs.Where(p => !pigStack.Contains(p)).ToList();
            for (int i = 0; i < pigsToProcess.Count; i++)
            {
                bool isLastPig = (i == pigsToProcess.Count - 1);
                ProcessPigData(pigsToProcess[i], pigStack.Count, null, skipRearrange: !isLastPig);
                pigStack.Add(pigsToProcess[i]);
                AssignPlateToPig(pigsToProcess[i]);
            }
            // RearrangeQueue(0, false);
        }
        else
    ...
    private void ProcessPigData(PigComponent pig, int count, Action onComplete = null, bool skipRearrange = false)
    {
        UIManager.Instance.UpdateStraightSlot(_straightSlot, _maxstraightSlot);
        ...
        if (pigsInQueue.Contains(pig) || pigsInTempQueue.Contains(pig))
        {
            ...
            HandlePigClickedFromQueue(pig, onComplete, isFromTempQueue, skipRearrange);
            return;
        }
    ...
    private void HandlePigClickedFromQueue(PigComponent pig, Action complete, bool isFromTempQueue = false, bool skipRearrange = false)
    {
        ...
        if (!skipRearrange && removedIndex >= 0)
    ...
    private void RearrangeQueue(int startIndex, bool isFromTempQueue = false)
    {
        ...
        else
        {
            for (int i = startIndex; i < pigsInTempQueue.Count; i++)
            {
                ...
                Vector3 targetPos = startTempQueuePos.position + 0.85f * i * Vector3.right;
    ...
    private void TryStartBottomPig()
    {
        if (pigStack.Count == 0) return;
